	...
	float fovy)
{
	...
	if (m_frameIndex >= FRAMES_IN_FLIGHT)
	{
		...
		{
			util::fatalExit("Failed to retrieve gpu timestamp queries!", EXIT_FAILURE);
		}
	...
	vkBeginCommandBuffer(curCmdBuf, &beginInfo);
	{
		...
		// gui renderpass
		{
            ...
            ImGui_ImplVulkan_RenderDrawData(ImGui::GetDrawData(), curCmdBuf);// show and render ImGui component(commented to hide!)
